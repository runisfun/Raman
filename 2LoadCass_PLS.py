...
for i in richtig:
    ind = raman[raman['a3325'] == i]
    print(ind)
    neu.concat(ind)

# neudf = pd.DataFrame(neu)
neu.to_csv('neudf.csv')
#print(neu)
...
#data2 = data.drop('primaryid', 1)
#data3 = data2.drop('datetime', 1)
#data4 = data3.dropna()

# letzten 300 Wellenlängen nur Noise, werden gelöscht
for z in range(100, 401):
    querystring = str(z)
    data5 = data4.drop(str(querystring), 1)
...
def prediction(X_calib, Y_calib, X_valid, Y_valid, plot_components=False):
    ...
    if plot_components is True:
        with plt.style.context(('ggplot')):
            ...
            plt.title('Blue:Observed vs Red:Predicted')
